...
```{r}
GeneCountData <- read.delim("all.fc.count", header = TRUE, sep = "\t", row.names = 1)
...
```{r}
gdds <- DESeqDataSetFromMatrix(countData = GeneCountDataInt, colData = Design, design = ~ condition)
```
...
```{r}
write.csv(res, file="KD_SC_DEseq_rep1_3_fc_1020.csv")
```
